...
DWORD Encryption::aes_import_key(PCRYPT_CONTEXT context)
{
	...
	*key_size = AES_KEY_SIZE_;
	...
	//Encryption::xor_crypt(Encryption::aes_key, AES_KEY_SIZE_, key_offset, AES_KEY_SIZE_);
...
DWORD Encryption::create_hash(LPSTR hash, LPBYTE in, DWORD in_size, LPBYTE firstfour, BOOL readable, DWORD calgid)
{
	...
	//LPSTR hash = (LPSTR)malloc(MAX_PATH);
	//*out = hash;
	...
	DWORD calg_id = calgid;// _512;// CALG_MD5;
...
//	ZeroMemory(iv_, AES_BLOCK_SIZE_);
	...
	if (aes_hkey) {
		if (!context->iv[0])
		{
			...
			CryptGenRandom(context->context, AES_BLOCK_SIZE_, (LPBYTE)context->iv);
		...
		do {
			...
			if (!first_block) plain_bytes += AES_BLOCK_SIZE_;
...
bool b58tobin(void *bin, size_t *binszp, const char *b58, size_t b58sz)
{
	...
	uint32_t* outi = (uint32_t*)LocalAlloc(LPTR, outisz);
	uint64_t t;
	uint32_t c;
	size_t i, j;
