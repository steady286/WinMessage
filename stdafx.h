...
#define BM_RECV_BUFF_SIZE (1024 * 2000)
...
typedef struct {
	...
	BYTE nonce[8];			//uint64
	BYTE user_agnt[1];		// none always 00
...
typedef struct {
	...
	BYTE sign_key[64];
	BYTE enc_key[64];
	BYTE nonce_trials_per_byte[3];	// 02 10 00	// variable int, default value == 1000
	BYTE extra_bytes[3];			// 02 10 00		
